...
class TranslationTracyPanel implements IBarPanel
{
    ...
    public function getPanel() : string {
        ...
        if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
            $accept = '<p><strong>'.lang(
                             'HTTP header',
                    context: 'debugPanel'
                ).':</strong> '.$_SERVER['HTTP_ACCEPT_LANGUAGE'].'</p>';
        }
        $panel = <<<HTML
        <h1>{$title}</h1>
        ...
        HTML;
        $panel .= '<p><strong>'.lang('Translated strings', context: 'debugPanel').':</strong> '.self::$translations.'</p>';
        foreach (self::$events as $event) {
            $panel .= '<div class="p-3 my-2 rounded border"><h5 class="my-1 fs-5">'.lang('Added a new string', context: 'debugPanel').':</h5><p>'.lang('String', context: 'debugPanel').': '.$event->message.'</p>';
            if (!empty($event->plural)) {
                $panel .= '<p>'.lang('Plural', context: 'debugPanel').': '.$event->plural.'</p>';
            }
            if (!empty($event->domain)) {
                $panel .= '<p>'.lang('Domain', context: 'debugPanel').': '.$event->domain.'</p>';
            }
            if (!empty($event->context)) {
                $panel .= '<p>'.lang('Context', context: 'debugPanel').': '.$event->context.'</p>';
            }
